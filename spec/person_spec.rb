...
describe Person do
  ...
  subject { described_class.new(name: 'Jesper') }
  ...
  describe 'can manage funds if an account been created' do
    ...
    it 'can withdraw funds' do
      command = lambda { subject.get_cash_from_atm(amount: 100, pin: subject.account.pin_code, account: subject.account, atm: atm) }
    ...
    it 'funds are added to cash - deducted from account balance' do
      allow(subject.account).to  receive(:exp_date).and_return('12/20')
      subject.cash = 100
      subject.deposit(100)
